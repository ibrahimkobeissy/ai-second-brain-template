...
- **Files one note per day** at `vault/02-personal/areas/second-brain/diaries/<YYYY-MM-DD>.md`. A second entry on the same day **appends** a new `## HH:MM` section — it never overwrites what's already there. A new day creates the note from the template (`type: daily`, linked to `[[second-brain]]` so it's in the graph).
...
| --- | --- | --- |
| Log today (or a named day) | `/diary <text>` | `diaries/<date>.md` |
