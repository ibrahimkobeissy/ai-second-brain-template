...
3. **Plan + persist the analysis plan.** Expand the set with transitive `depends_on`, then topologically sort into levels (see conventions → *Execution planning & concurrency*). Abort on a cycle. **The level/run order is the file numbering** (next step). **Before running anything, write the plan into `00-productization-plan.md`** under an `## Analysis plan` section, rendered as an **ASCII dependency map inside a fenced code block** (a diagram, *not* prose):
   - a header/legend line: `◄── = depends on · run order = NN (top → bottom)`;
   - one **level band** per topological level (`LEVEL 0 · run NN–NN`, `LEVEL 1 · …`, …) top-to-bottom = run order, with a `▼` between bands to show the flow;
   - under each band, one line per analysis: `NN  <id>   ◄── <upstream NNs>` (use `no upstream`, or `(standalone)` when it has neither deps nor dependents);
