...
Every phase skill, as its **final step**, updates the product's `productization_plan.md` (mark the phase row, refresh the artifact-link table, bump `updated`). `productize-analyze` additionally writes an **`## Analysis plan`** — an **ASCII dependency map** — into the control file *before* it runs, so the planned run is legible up front rather than left implicit. `productize-plan` is the **manual fallback** — it regenerates the file purely from the files present on disk (reconstructing the `## Analysis plan` from `03-analyses/` `depends_on`), so a drifted or hand-edited product can always be re-synced. **Truth is the filesystem**, never assumed state; links only ever point at files that exist.

### The `## Analysis plan` map — one rendering algorithm (shared by analyze + plan)
The generator (`productize-analyze`, from the selected set) and the reconstructor (`productize-plan`, from `03-analyses/` on disk) **must render the same map from the same inputs**, so the map can be re-derived and machine-checked (productize-linter **P6**). The map is a function of one thing — each analysis's **`depends_on`** — never of the `NN` filenames:
1. **Levels come from `depends_on`, not from `NN`.** A node's level is its longest dependency chain (ASAP layering): `level = 0` if `depends_on` is empty, else `1 + max(level of each dep)`. One **level band** per level, top (0) → bottom = run order; **abort on a cycle**. *(`NN` is a total order and cannot encode the level partition — never reconstruct bands by slicing the `NN` sequence.)*
2. **`NN` is the run-order label only.** Walk the bands top-down and number analyses `01, 02, …` = read/run position. It labels rows and edges (`◄── NN`); it is never the basis for which band a node sits in.
3. **`(standalone)` = no deps **and** no dependents.** A node whose `depends_on` is empty **and** which no other node lists in its `depends_on` is isolated; tag it `(standalone)` **inline, immediately after its own `id`** (so the tag is unambiguous when a band line holds several ids). A Level-0 node that *is* depended upon is `no upstream`, **not** standalone. *(Standalone nodes still sit in Level 0 — empty deps — but carry the isolated-node tag.)*
